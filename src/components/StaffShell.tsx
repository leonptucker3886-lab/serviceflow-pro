    ...
    <div className="min-h-screen flex bg-slate-50">
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 bg-white border-r border-slate-200 fixed inset-y-0">
        ...
        <div className="p-4 border-t border-slate-100">
          ...
          <div className="font-medium text-slate-900 text-sm truncate">{currentUser.name}</div>
          <div className="text-xs text-slate-500 capitalize">{currentUser.role}</div>
          <button
            onClick={handleLogout}
            className="mt-3 text-sm text-slate-500 hover:text-red-600 transition"
          >
            Sign out
          </button>
        </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-30 bg-black/40" onClick={() => setMobileOpen(false)}>
          <div
            className="absolute top-14 left-0 right-0 bg-white border-b border-slate-200 p-4 space-y-1 shadow-lg"
